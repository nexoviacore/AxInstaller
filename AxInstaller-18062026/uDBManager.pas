...
uses
  Windows , SysUtils, Classes, dialogs, ADODb, Types,
  Forms, Variants, XMLDoc, XMLIntf,  IdGlobal,
  uGeneralFunctions, uConnect, uXDS , DateUtils,Controls,
  Data.DBXOracle, Data.DB, Data.SqlExpr,dbxcommon, DbxDevartMySql,DbxDevartPostgreSQL,uCompress, uAxLog ;
...
begin
  try
    writelog('ConnectToDatabase started. ConnectionName=' + ConnectionName);
    gf.dbtimetaken := 0;
    errorstr := '';
    Result := false;
    if ConnectionName = '' then
    begin
      writelog('ConnectionName empty. Exiting ConnectToDatabase.');
      exit;
    end;
    if gf.exepath = '' then
      gf.exepath := GetCurrentDir; {AppDir;}
    ConnectionName :=trim(connectionname);
    gf.dodebug.Log('Requested ConnectionName = ' + ConnectionName);
    if gf.RemoteLogin then
    begin
      writelog('RemoteLogin has been initiated.');
       connect.RemoteConnect(connectionname);
       Connection := Connect.Connection;
       if gf.remotedbType = 'access' then
       begin
         writelog('Remote DB type : access .');
         gf.sqllower:='lcase';
         gf.forupdate :=' for update ';
         gf.sqlnull := 'nvl';
       end else if gf.remotedbType = 'ms sql' then
       begin
         writelog('Remote DB type : ms sql .');
         gf.sqllower:='lower';
       ...
       begin
          writelog('Remote DB type : oracle .');
          gf.sqllower:='lower';
          gf.forupdate :=' for update ';
          gf.sqlnull := 'nvl';
       ...
       begin
          writelog('Remote DB type : postgre .');
          gf.sqllower:='lower';
          gf.forupdate :=' for update ';
          gf.sqlnull := 'coalesce';
    ...
    writelog('Creating connection object...');
    if assigned(conXML) then
    Connect.conXML := conXML;
    ...
    begin
      ...
      begin
        ...
        begin
          //ReadErrorList(e.Message);
          writelog('Retry connection failed: ' + e.Message);
    ...
    writelog('Connection established. DBType=' + gf.dbase + ', Schema=' + gf.schemaname);
    gf.dodebug.Msg('creating connection end...');
    gf.dodebug.Msg('creating date xds...');
    writelog('Creating TXDS ...');
    wdt := TXDS.create('wdt', nil, Connection,gf);
    wdt.buffered:=true;
    gf.dodebug.Msg('creating date xds end...');
    writelog('Configuring DB specific settings for DBType=' + connection.DbType);
    if connection.DbType = 'access' then begin
    ...
    if (errorstr = '') and (GetServerDT) then
    begin
      writelog('Fetching server datetime...');
      ServerDateTime := GetServerDateTime;
    end;
    if (errorstr = '') and (remoteOpen) then
    begin
    ...
    if errorstr <> '' then
    begin
      writelog('Connection error: ' + errorstr);
      raise exception.Create(errorstr);
    end;
    if (errorstr='') and (SetDefaultValues) then
    begin
      writelog('Applying default DB values...');
      SetDefaults;
    end;
    gf.DoDebug.Msg('ConnectToDatabase function ends successfully.');
  except
    on E: Exception do
    begin
      //ReadErrorList(E.Message);
      writelog('Exception in ConnectToDatabase: ' + E.Message);
      raise;
    end;
  end;
end;
...
begin
  ...
  q:=getxds(nil);
  q.buffered := true;
  q.CDS.CommandText:='select '+fname+' from '+table+' where '+where+' order by blobno';
  q.open;
  i := q.CDS.RecordCount;
  if assigned(q) then
  begin
